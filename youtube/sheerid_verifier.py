...
class SheerIDVerifier:
    ...
    def parse_verification_id(url: str) -> Optional[str]:
        ...
        支持标准 SheerID URL 和 YouTube offers URL
        """
        # 标准 SheerID URL: ?verificationId=xxx
    ...
    @staticmethod
    def create_verification_from_youtube_url(youtube_url: str) -> Optional[Tuple[str, str]]:
        """
        从 YouTube offers URL 创建新的验证会话

        YouTube URL 格式: https://offers.sheerid.com/youtube/student/?...

        Returns:
            Tuple[programId, verificationId] 或 None
        """
        try:
            # 检查是否是 YouTube offers URL
            if "offers.sheerid.com/youtube" not in youtube_url:
                return None

            # 发起请求获取 verificationId
            # 这需要模拟浏览器访问并抓取 API 响应
            # 由于复杂性，建议用户手动提取
            logger.warning("YouTube offers URL 需要手动提取 verificationId")
            return None

        except Exception as e:
            logger.error(f"解析 YouTube URL 失败: {e}")
            return None
